...
class OptimGradientBoosting(Ensemble):
    ...
    def fit(self, X, y, relation, sample_weight=None, eval_sets=None, relation_eval=None):
        ...
        for val_arr in eval_sets:
            if 'sample_weight' not in val_arr:
                val_arr['sample_weight'] = None
            val_arr['X'], val_arr['y'], val_arr['sample_weight'] = validate_input_single(val_arr['X'], val_arr['y'], val_arr['sample_weight'])
        ...
        with cp.cuda.using_allocator(allocator=mempool.malloc):
            ...
            # create build info
            builder, build_info = self._create_build_info(mempool, X, relation, X_enc, y, sample_weight, max_bin, borders, eval_sets, relation_eval, eval_enc)
            self._fit(builder, build_info)
        mempool.free_all_blocks()

        return self

    def _create_build_info(self, mempool, X, relation, X_enc, y, sample_weight, max_bin, borders, eval_sets, relation_eval, eval_enc):
