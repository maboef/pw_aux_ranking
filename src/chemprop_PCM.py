...
def train_chemprop_PCM(train, valid, model_path: str,  **kwargs):
    """
    ...
    datapoints = [data.MoleculeDatapoint(mol=mol, y=[float(y)], x_d=prot) for mol, y, prot in zip(mols, ys, prots)]
    ...
    ffn = nn.RegressionFFN(hidden_dim=params['hidden_size'], input_dim = ffn_input_dim, n_tasks = 1, n_layers=params['ffn_num_layers'], dropout=params['dropout'], activation=params['activation'], criterion=metrics.RMSE())
